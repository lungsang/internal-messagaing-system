...
class Thread extends Eloquent implements HasMedia
{
    ...
    public function getLatestMessageAttributeForUser()
    {
     ...
     /**
     * Returns the latest message from a thread which is written by the logged in user.
     *
     * @return null|\Cmgmyr\Messenger\Models\Message
     */
    ...
    /**
     * Returns threads that the user is associated with which are deleted.
     *
     * @param \Illuminate\Database\Eloquent\Builder $query
     * @param int $userId
     *
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function scopeTrashForUser(Builder $query, $userId)
    {
        $participantsTable = Models::table('participants');
        $threadsTable = Models::table('threads');
        ...
        return $query->join($participantsTable, $this->getQualifiedKeyName(), '=', $participantsTable . '.thread_id')
            ->where($participantsTable . '.user_id', $userId)
            ->whereNotNull($participantsTable . '.deleted_at')
            ->select($threadsTable . '.*');
    }

    /**
     * Returns threads with new messages (unread) that the user is associated with.
     *
     * @param \Illuminate\Database\Eloquent\Builder $query
     * @param int $userId
     *
